          ...
          <div className="space-y-8">
            {groups.map(({ section, id, items }) => {
              return (
                <section key={id}>
                  {section && (
                    <h2 className="flex items-baseline gap-2 text-xs font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500 mb-3 pb-1 border-b border-gray-100 dark:border-gray-800">
                      <span className="text-2xl font-black text-amber-400 dark:text-amber-500 leading-none select-none">#</span>
                      {section.name}
                    </h2>
                  )}
                  {!section && hasSections && (
